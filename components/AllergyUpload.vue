<template>
  <div v-if="translations">
    <div class="allergy-upload">
      <QStepper  v-model="step"
                  ref="stepper"
                  color="primary"
                  animated>
        <QStep :name="1"
                :title="translations.substance"
                :done="step > 1">
          <div class="row">
            <div class="col no-allergies-checkbox">
              <QCheckbox v-model="noAllergiesCheckbox" size="sm" />
            </div>

            <div class="col no-allergies-checkbox-text">
              <p class="no-allergies-text">{{ translations.noKnownAllergies }}</p>
            </div>
          </div>

          <div class="row" v-if="noAllergiesCheckbox">
            <div class="col">
              <QSelect v-model="noAllergiesSelect"
                      ...
                      :label="translations.noKnownAllergiesMoreDetail  + ' *'"
                      dense/>
            </div>
          </div>
          ...
          <div class="row" v-if="!noAllergiesCheckbox">
            ...
            <div class="tooltip">
              <QIcon  name="fas fa-question-circle">
                <QTooltip>{{ translations.typeTooltip }}</QTooltip>
              </QIcon>
            </div>
          ...
          <div class="row" v-if="!noAllergiesCheckbox">
            <QSelect v-model="allergyCodeSelect"
                      :rules="[val => !!val || translations.fieldRequired ]"
                      :options="allergyCodeFilteredOptions"
                      :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
                      :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
                      @filter="filterAllergyCodeSelect"
                      :label="translations.allergicFor + ' *'"
                      stack-label
                      :placeholder="allergyCodeSelect ? '' : translations.searchPlaceholder"
                      class="input-with-tooltip"
                      use-input
                      dense
                      clearable>
              <template v-slot:no-option>
                <!-- TODO: this slot produces Uncaught TypeError in use-virtual-scroll.js why? -->
                <QItem>
            ...
            <div class="tooltip">
              <QIcon name="fas fa-question-circle">
                <QTooltip>{{ translations.substanceTooltip }}</QTooltip>
              </QIcon>
            </div>
          </div>

          <div class="row" v-if="!noAllergiesCheckbox">
            <QSelect v-model="allergyCategorySelect"
            ...
            <div class="tooltip">
              <QIcon name="fas fa-question-circle">
                <QTooltip>{{ translations.categoryTooltip }}</QTooltip>
              </QIcon>
            </div>
          </div>
          ...
          <div class="row">
            <QSelect
              ...
              :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
              :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
              :label="translations.clinicalStatus + (needsClinicalStatus ? '*' : '')"
              class="input-with-tooltip"
              dense
              clearable/>
            <div class="tooltip">
              <QIcon name="fas fa-question-circle">
                <QTooltip>{{ translations.clinicalStatusTooltip }}</QTooltip>
              </QIcon>
            </div>
          </div>
          ...
          <div class="row" v-if="!noAllergiesCheckbox">
            <QSelect
              ...
              :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
              :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
              :label="translations.criticality"
              class="input-with-tooltip"
              dense
              clearable/>
            <div class="tooltip">
              <QIcon name="fas fa-question-circle">
                <QTooltip>{{ translations.criticalityTooltip }}</QTooltip>
              </QIcon>
            </div>
          </div>
          ...
          <div class="row">
            <QSelect
              ...
              :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
              :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
              :label="translations.verificationStatus"
              class="input-with-tooltip"
              dense
              clearable/>
            <div class="tooltip">
              <QIcon name="fas fa-question-circle">
                <QTooltip>{{ translations.verificationStatusTooltip }}</QTooltip>
              </QIcon>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col">
              <QInput
                ...
                :label="translations.dateOfIdentification"
                dense>
                <template v-slot:append>
                  <QIcon name="fas fa-calendar-day">
                    <QPopupProxy cover transition-show="scale" transition-hide="scale">
                      <QDate v-model="identificationDateInput" mask="YYYY-MM-DD" />
                    </QPopupProxy>
                  </QIcon>
                </template>
              </QInput>
        ...
        <QStep :name="2"
                :title="translations.episode"
                :done="step > 2"
                v-if="!noAllergiesCheckbox">

          <template v-if="!isAddEpisodeFormShown">
            <QTable  :title="translations.episodeTableTitle"
                      :rows="episodes"
            ...
            <QBtn flat
                  ...
                  color="primary"
                  :label="translations.addEpisodeButtonLabel" />
          </template>

          <div v-if="isAddEpisodeFormShown">
            <div class="section">
              <h3>{{ translations.reaction }}</h3>
              <div class="col">
                <QInput
                  v-model="reactionDateInput"
                  :label="translations.reactionDate"
                  dense>
                  <template v-slot:append>
                    <QIcon name="fas fa-calendar-day">
                      <QPopupProxy cover transition-show="scale" transition-hide="scale">
                        <QDate v-model="reactionDateInput" mask="YYYY-MM-DD" />
                      </QPopupProxy>
                    </QIcon>
                  </template>
                </QInput>
              ...
              <div class="row">
                <div class="col">
                  <QInput
                    ...
                    type="number"
                    :rules="[val => (!val || (val == 0 || val > 0)) || translations.positiveValuesAllowed]"
                    dense
                  />
                </div>
              </div>
              ...
              <div class="row">
                <QSelect v-model="reactionSubstanceSelect"
                          :options="reactionSubstanceFilteredOptions"
                          :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
                          :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
                          @filter="filterReactionSubstanceSelect"
                          :label="translations.substance"
                          :placeholder="reactionSubstanceSelect ? '' : translations.searchPlaceholder"
                          class="input-with-tooltip"
                          use-input
                          dense
                          clearable>
                ...
                <div class="tooltip">
                  <QIcon name="fas fa-question-circle">
                    <QTooltip>{{ translations.reactionSubstanceTooltip }}</QTooltip>
                  </QIcon>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col">
                   <QSelect  v-model="reactionCertaintySelect"
                              :options="reactionCertaintyOptions"
                              :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
                              :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
                              :label="translations.certainty"
                              dense
                              clearable/>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col">
                  <QSelect v-model="reactionSeveritySelect"
                            :options="reactionSeverityOptions"
                            :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
                            :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
                            :label="translations.reactionSeverity"
                            dense
                            clearable/>
                </div>
              </div>
              ...
              <div class="row">
                <QSelect v-model="reactionManifestationSelect"
                          :rules="[val => !!val || translations.fieldRequired ]"
                          :options="reactionManifestationFilteredOptions"
                          :option-label="(item) => (item == null ? '?' : item.languageDisplays[languageString])"
                          :option-value="(item) => (item == null ? null : item.defaultCoding.code)"
                          @filter="filterReactionManifestationSelect"
                          :label="translations.manifestation + ' *'"
                          :placeholder="reactionManifestationSelect ? '' : translations.searchPlaceholder"
                          class="input-with-tooltip"
                          use-input
                          multiple
                          dense
                ...
                <div class="tooltip">
                  <QIcon name="fas fa-question-circle">
                    <QTooltip>{{ translations.reactionManifestationTooltip }}</QTooltip>
                  </QIcon>
                </div>
              </div>

              <div class="row">
                <QSelect v-model="reactionLocationSelect"
                          ...
                          :placeholder="reactionLocationSelect ? '' : translations.searchPlaceholder"
                          class="input-with-tooltip"
                          use-input
                          dense
                          clearable>
                ...
                <div class="tooltip">
                  <QIcon name="fas fa-question-circle">
                    <QTooltip>{{ translations.reactionLocationTooltip }}</QTooltip>
                  </QIcon>
                </div>
              </div>
            </div>
            ...
            <div class="section">
              <h3>{{ translations.exposure }}</h3>

              <div class="col">
                <QInput
                  v-model="exposureDateInput"
                  :label="translations.exposureDate">
                  <template v-slot:append>
                    <QIcon name="fas fa-calendar-day">
                      <QPopupProxy cover transition-show="scale" transition-hide="scale">
                        <QDate v-model="exposureDateInput" mask="YYYY-MM-DD" />
                      </QPopupProxy>
                    </QIcon>
                  </template>
                </QInput>
              ...
              <div class="row">
                <QSelect v-model="exposurePathSelect"
                          :options="exposurePathFilteredOptions"
                          :option-label="(item) => (item == null ? '?' : item.display)"
                          :option-value="(item) => (item == null ? null : item.code)"
                          @filter="filterExposurePathSelect"
                          :label="translations.exposurePath"
                          :placeholder="exposurePathSelect ? '' : translations.searchPlaceholder"
                          class="input-with-tooltip"
                          use-input
                          dense
                          clearable>
                ...
                <div class="tooltip">
                  <QIcon name="fas fa-question-circle">
                    <QTooltip>{{ translations.exposurePathTooltip }}</QTooltip>
                  </QIcon>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col">
                  <QInput
                    ...
                    :label="translations.episodeComment"
                    autogrow
                    filled
                    dense
                  />
            ...
            <div class="button-container">
              <QBtn @click="closeEpisodeForm()" :label="translations.cancelButtonLabel" />
              <QBtn @click="deleteEpisode()"
                    v-if="isEpisodeInEditMode"
                    :label="translations.deleteEpisodeButtonLabel"/>
              <QBtn @click="setEpisodeValues(true)"
                    v-if="!isEpisodeInEditMode"
                    :disabled="!readyAddEpisode"
                    color="primary"
                    :label="translations.saveEpisode"
                    class="q-ml-sm"/>
              <QBtn @click="setEpisodeValues(false)"
                    v-if="isEpisodeInEditMode"
                    :disabled="!readyAddEpisode"
                    color="primary"
                    :label="translations.updateEpisodeButtonLabel"
                    class="q-ml-sm"/>
            </div>
          </div>
          ...
          <QStepperNavigation v-if="!isAddEpisodeFormShown">
            ...
            <QBtn @click="() => { step = 3 }" color="primary" :label="translations.continue" :disable="false" class="q-ml-sm"/>
          </QStepperNavigation>
        </QStep>
        ...
        <QStep :name="3"
          ...
          <QStepperNavigation>
            <QBtn flat @click="backFromLastStep()" color="primary" :label="translations.back" />
          </QStepperNavigation>
        </QStep>
      </QStepper>

      <div class="button-container">
        <QBtn @click="() => onDone(false)" :label="translations.cancelButtonLabel" />
        <QBtn :disabled="!readyForUpload" @click="uploadAllergy" :color="readyForUpload ? 'primary' : undefined"
...
import { QStepper, QStep, QCheckbox, QSelect, QIcon, QTooltip, QItem, QItemSection, QInput, QPopupProxy, QBtn, QDate, QStepperNavigation, QTable } from 'quasar';
import { 
...
export default defineComponent({
  name: 'AllergyUpload',
  components: { QStepper, QStep, QCheckbox, QSelect, QIcon, QTooltip, QItem, QItemSection, QInput, QPopupProxy, QBtn, QDate, QStepperNavigation, QTable },
  data() {
    return {
      ...
      episodeColumns: [               // definitions of the episode table columns
        {
          name: 'reactionDate',
          required: true,
          label: this.$props.translations.reactionDate,
          align: 'left',
        ...
        },
        {
          name: 'duration',
          required: false,
          label: this.$props.translations.durationReaction,
          align: 'left',
        ...
        },
        {
          name: 'substance',
          required: true,
          label: this.$props.translations.substance,
          align: 'left',
          ...
          format: (code: CodeableConcept) => {
            if (code && code.coding && code.coding.length > 0 && code.coding[0].code) {
              const translated = this.fhirUtils.getDisplayByCodeAndLanguage(code.coding[0].code, REACTION_SUBSTANCE_CODES,this.languageString);
        ...
        },
        {
          name: 'manifestation',
          required: false,
          label: this.$props.translations.manifestation,
          align: 'left',
            ...
            if (codes && codes.length > 0) {
              ...
              codes.forEach(code => {
                if (code.coding && code.coding.length > 0 && code.coding[0].code) {
                  translated = translated + this.fhirUtils.getDisplayByCodeAndLanguage(code.coding[0].code, REACTION_MANIFESTATION_CODES, this.languageString);
                }
                if (codes.length > 1) {
                  // add a comma
        ...
        },
        {
          name: 'exposureDate',
          required: false,
          label: this.$props.translations.exposureDate,
          align: 'left',
        ...
        },
        {
          name: 'severity',
          required: true,
          label: this.$props.translations.reactionSeverity,
          align: 'left',
        ...
        },
        {
          name: 'description',
          required: false,
          label: this.$props.translations.description,
          align: 'left',
  ...
  beforeMount() {
    ...
    [ this.reactionLocationOptions, this.exposurePathOptions ]
    .forEach(optionsArray => {
      optionsArray.sort((a, b): number => {
  ...
  methods: {
    ...
    setFormData() {
      ...
      if (bundle) {
        ...
        const indexNoAllergies = this.noAllergiesOptions.findIndex(option => option.defaultCoding.code == bundle.code);
        if (bundle.code && indexNoAllergies >= 0) {
          ...
          // there is an allergy
          const allergyCode = this.allergyCodeOptions.find(option => option.defaultCoding.code == bundle.code);
          if (allergyCode) {
          ...
          if (bundle.category && bundle.category.length > 0) {
            ...
            bundle.category.forEach(category => {
              const code = this.allergyCategoryOptions.find(option => option.defaultCoding.code == category) as SystemCodeExtension;
          ...
          if (bundle.clinicalStatus) {
            this.clinicalStatusSelect = this.clinicalStatusOptions.find((option) => {
              if (bundle.clinicalStatus && bundle.clinicalStatus.coding && bundle.clinicalStatus.coding.length > 0 && bundle.clinicalStatus.coding[0] && bundle.clinicalStatus.coding[0].code && bundle.clinicalStatus.coding[0].code == option.defaultCoding.code) {
                return option;
              }
            });
          }
          ...
          if (bundle.criticality) {
            this.criticalitySelect = this.criticalityOptions.find(option => option.defaultCoding.code == bundle.criticality);
          }

          if (bundle.verificationStatus) {
            this.verificationStatusSelect = this.verificationStatusOptions.find((option) => {
              if (bundle.verificationStatus && bundle.verificationStatus.coding && bundle.verificationStatus.coding.length > 0 && bundle.verificationStatus.coding[0] && bundle.verificationStatus.coding[0].code && bundle.verificationStatus.coding[0].code == option.defaultCoding.code) {
                return option;
              }
            });
          }
        ...
        if (bundle.reaction) {
          // TODO: add episode data to episodes table, use setEpisodeValues()
        }

        // TODO: set data of creating institution
      } else {
    ...
    setEpisodeValues(isNewEpisode: boolean) {
      ...
      const episode: AllergyIntoleranceReactionCH = {
        ...
        onset: this.reactionDateInput.length > 0 
          ? this.reactionDateInput
          : undefined,
        description: this.reactionDescriptionInput.length > 0 
          ? this.reactionDescriptionInput
          : undefined,
        manifestation: this.reactionManifestationSelect?.map(selected => {
          return {
            coding: [
              selected.defaultCoding
            ]
          }
        }) || [],
        allergyintoleranceDuration: this.reactionDurationInput !== undefined
          ? {
        ...
        substance: this.reactionSubstanceSelect
          ? { 
              coding: [
                this.reactionSubstanceSelect.defaultCoding
              ] 
            }
          : undefined,
        allergyintoleranceCertainty: this.reactionCertaintySelect
          ? { 
              coding: [
                this.reactionCertaintySelect.defaultCoding
              ] 
            }
          : undefined,
        openEHRExposureDate: this.exposureDateInput.length > 0
          ? this.exposureDateInput
          : undefined,
        openEHRExposureDescription: this.exposureDescriptionInput.length > 0
          ? this.exposureDescriptionInput
          : undefined,
        openEHRLocation: this.reactionLocationSelect 
          ? { 
              coding: [
                this.reactionLocationSelect
              ]
            }
          : undefined,
        openEHRExposureDuration: this.exposureDateInput !== undefined 
          ? {
        ...
        exposureRoute: this.exposurePathSelect
          ? {
              coding: [
                this.exposurePathSelect
              ]
            }
          : undefined,
        note: this.episodeCommentInput.length > 0
          ? [
              {
    ...
    selectEpisode(event: Event, episode: AllergyIntoleranceReactionCH) {
      ...
      if (episode.manifestation && episode.manifestation.length > 0) {
        ...
        episode.manifestation.forEach(manifestation => {
          let manifestationCode = this.reactionManifestationOptions.find((option) => {
            if (manifestation.coding && manifestation.coding[0].code && option.defaultCoding.code == manifestation.coding[0].code) {
      ...
      if (episode.substance) {
        let substanceCode = this.reactionSubstanceOptions.find((option) => {
          if (episode.substance?.coding && episode.substance.coding.length > 0 && episode.substance.coding[0].code == option.defaultCoding.code) {
            return option;
          }
        });
        if (substanceCode) {
      ...
      if (episode.allergyintoleranceCertainty) {
        let certaintyCode = this.reactionCertaintyOptions.find((option) => {
          if (episode.allergyintoleranceCertainty?.coding && episode.allergyintoleranceCertainty.coding.length > 0 && episode.allergyintoleranceCertainty.coding[0].code == option.defaultCoding.code) {
            return option;
          }
        });
        if (certaintyCode) {
     ...
     * Calls the onDone method and emits an upload-result event.
     */
    uploadAllergy() {
      if ((!this.allergyCodeSelect && !this.noAllergiesCheckbox) || this.noAllergiesCheckbox && !this.noAllergiesSelect) {
      ...
      const allergyParams: AllergyIntoleranceParams = {
        ...
        clinicalStatus: this.clinicalStatusSelect 
          ? { coding: [ this.clinicalStatusSelect.defaultCoding ] }
          : undefined,
        ...
        verificationStatus: this.verificationStatusSelect
          ...
          : undefined,
        onsetDateTime: this.identificationDateInput.length > 0 
          ? this.identificationDateInput 
          : undefined,
        note: this.commentInput && this.commentInput.length > 0
      ...
      if (allergyResource && allergyResource.code && allergyResource.code.coding && allergyResource.code.coding.length > 0) {
        const documentBundleTitle = allergyResource.code.coding[0].display;
        ...
        const metadata = {
          ...
          isFhir: true,
          description: allergyResource.code.coding[0].display,
        ...
        this.fhirUtils.createIti65Bundle(this.patient, allergyInFile, metadata)
  ...
  computed: {
    ...
    readyAddEpisode(): boolean {
      if (this.reactionManifestationSelect && (!this.reactionDurationInput || this.reactionDurationInput >= 0) && (!this.exposureDurationInput || this.exposureDurationInput >= 0)) {
        return true;
      }
      return false;
    },
